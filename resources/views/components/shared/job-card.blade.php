@props(['job', 'first' => false, 'small' => false])
...
@if ($small)
    <article
        ...
        <div @class([
            'absolute top-0 left-3 right-3 h-[3px] rounded-t-xl bg-primary transition-opacity duration-200',
            'opacity-100' => $first,
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        @auth
                            @if (auth()->user()->role->value === 'seeker')
                                <form action="{{ route('seeker.bookmark.store', $job) }}" method="POST" @click.stop>
                                    @csrf
                                    <button type="submit"
        ...
        <div class="hidden sm:grid sm:grid-cols-[auto_1fr_auto_auto] sm:gap-4 sm:items-center p-5 pl-6">
            <div class="avatar shrink-0">
                <div class="w-12 h-12 rounded-xl bg-base-200">
                    ...
                    @else
                        <span
                            class="text-base-content/40 font-bold text-sm flex items-center justify-center w-full h-full">
                            {{ strtoupper(substr($job->company->name, 0, 2)) }}
                        </span>
                    @endif
                </div>
